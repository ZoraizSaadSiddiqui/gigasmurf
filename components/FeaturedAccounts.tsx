...
const navigationStyle: CSSProperties = {
  ...
  height: '40px',
  padding: 0,
  border: '1px solid rgba(160, 35, 236, 0.45)',
  borderRadius: '50%',
  background: 'transparent',
  color: '#fff',
  fontSize: '21px',
...
export default function FeaturedAccounts() {
  ...
  const moveCarousel = useCallback((direction: 1 | -1) => {
    ...
    carousel.scrollTo({ left: direction === 1 ? (atEnd ? 0 : carousel.scrollLeft + distance) : (atStart ? carousel.scrollWidth - carousel.clientWidth : carousel.scrollLeft - distance), behavior: 'smooth' });
  }, []);
        ...
        <div className="d-flex align-items-center justify-content-between flex-nowrap" style={headerStyle}>
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="d-flex gap-2">
              ...
              <button type="button" onClick={() => moveCarousel(1)} aria-label="Next featured account" className="btn slider-navigation-button d-flex align-items-center justify-content-center" style={navigationStyle}>&rsaquo;</button>
            </div>
          </div>
        </div>
